...
OpenposeWrapper::OpenposeWrapper(void):opWrapper{op::ThreadManagerMode::Asynchronous}
{
    // logging_level
	...
	data = std::make_shared<std::vector<UserDatum>>();
	data->emplace_back();
}

std::shared_ptr<std::vector<UserDatum> > OpenposeWrapper::process( cv::Mat &img )
{

	auto& datum = data->at(0);
	datum.cvInputData = img;

	std::shared_ptr<std::vector<UserDatum> > datumProcessed;
	auto ret = opWrapper.waitAndEmplace(data);

	if ( ret && opWrapper.waitAndPop(datumProcessed) )
	{
