...
#include <openpose_ros_wrapper_msgs/PersonsArray.h>
...
void image_callback(const sensor_msgs::ImageConstPtr &img) {
    ...
    if (output) {
        openpose_ros_wrapper_msgs::PersonsArray psarr;
        psarr.persons = mp->callback(output);
        psarr.header.stamp = img->header.stamp;

        pose_pub->publish(psarr);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    ros::Publisher pub = nh.advertise<openpose_ros_wrapper_msgs::PersonsArray>("/openpose/pose", 1);
    pose_pub = &pub;
